...
class Actor(nn.Module):
    def __init__(self, args):
        ...
        self.fc_mu = nn.Linear(args.hidden_dim, args.action_dim_Cannon[0])
        self.fc_std = nn.Linear(args.hidden_dim, args.action_dim_Cannon[0])
        self.action2 = nn.Linear(args.hidden_dim, args.action_dim_Cannon[1])
    ...
    def forward(self, inputs, h_in):
        ...
        action2 = F.softmax(self.action2(h_out), dim = -1)
...
class Mappo_Cannon:
    ...
    def save_model(self, env_name, number, seed, total_steps):
        torch.save(self.actor.state_dict(), "C:/Workspace/WRJ/MACA-2D/MACA/algorithm/mappo/model/Cannon_actor_env_{}_number_{}_seed_{}_step_{}.pth".format(env_name, number, seed, int(total_steps)))

    def load_model(self, env_name, number, seed, step):
        self.actor.load_state_dict(torch.load("C:/Workspace/WRJ/MACA-2D/MACA/algorithm/mappo/model/Cannon_actor_env_{}_number_{}_seed_{}_step_{}.pth".format(env_name, number, seed, step)))
