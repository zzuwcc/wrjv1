...
class Actor(nn.Module):
    def __init__(self, args, subtask_select):
        ...
        self.fc_mu_w = nn.Linear(args.subtask_emb_dim, args.hidden_dim * args.action_dim_Reconn)
        self.fc_mu_b = nn.Linear(args.subtask_emb_dim, args.action_dim_Reconn)
        self.fc_std_w = nn.Linear(args.subtask_emb_dim, args.hidden_dim * args.action_dim_Reconn)
        self.fc_std_b = nn.Linear(args.subtask_emb_dim, args.action_dim_Reconn)
    ...
    def forward(self, inputs, h_in):
        ...
        mu_w = self.fc_mu_w(subtask_emb).reshape(-1, self.args.hidden_dim, self.args.action_dim_Reconn)
        mu_b = self.fc_mu_b(subtask_emb).reshape(-1, self.args.action_dim_Reconn)
        std_w = self.fc_std_w(subtask_emb).reshape(-1, self.args.hidden_dim, self.args.action_dim_Reconn)
        std_b = self.fc_std_b(subtask_emb).reshape(-1, self.args.action_dim_Reconn)
...
class Mappo_Reconn:
    ...
    def save_model(self, env_name, number, seed, total_steps):
        torch.save(self.actor.state_dict(), "C:/Workspace/WRJ/MACA-2D/MACA/algorithm/subtask_mappo/model/Reconn_actor_env_{}_number_{}_seed_{}_step_{}k.pth".format(env_name, number, seed, int(total_steps)))

    def load_model(self, env_name, number, seed, step):
        self.actor.load_state_dict(torch.load("C:/Workspace/WRJ/MACA-2D/MACA/algorithm/subtask_mappo/model/Reconn_actor_env_{}_number_{}_seed_{}_step_{}k.pth".format(env_name, number, seed, step)))
