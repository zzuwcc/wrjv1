...
class RunnerMACA:
    def __init__(self, args, env_name):
        ...
        self.subtask_select = SubtaskSelect(self.args)
        self.agent_Reconn = Mappo_Reconn(self.args, self.subtask_select)
        self.agent_Cannon = Mappo_Cannon(self.args, self.subtask_select)
    ...
    def run(self):
        with open('C:/Workspace/WRJ/MACA-2D/MACA/algorithm/subtask_mappo/result/hyper_param.txt', 'w') as f:
            f.write(f'Actor input dim: {self.args.actor_input_dim}\n')
            f.write(f'Critic input dim: {self.args.critic_input_dim}\n')
            f.write(f'Action dim Reconn: {self.args.action_dim_Reconn}\n')
            f.write(f'Action dim Cannon: {self.args.action_dim_Cannon}\n')
            f.write(f'N: {self.args.N}\n')
            f.write(f'N_Reconn: {self.args.N_Reconn}\n')
            f.write(f'N_Cannon: {self.args.N_Cannon}\n')
            f.write(f'Obs dim: {self.args.obs_dim}\n')
        evaluate_times = 0
        reward_mean_record = []
        while self.update_steps < self.args.total_steps:
            episode_reward, _ = self.run_episode(evaluate=False)
            ...
            if self.update_steps >= evaluate_times * self.args.evaluate_cycle:
                ...
                with open("C:/Workspace/WRJ/MACA-2D/MACA/algorithm/subtask_mappo/result/log_{}.txt".format(self.number), "a") as f:
                ...
                self.subtask_select.save_model(self.env_name, self.number, self.seed, self.update_steps)
        
        # save the reward_mean_record to .npy file
        np.save("C:/Workspace/WRJ/MACA-2D/MACA/algorithm/subtask_mappo/result/evaluate_reward_mean_record_{}.npy".format(self.number), reward_mean_record)
    ...
    def run_episode(self, evaluate=False):
        ...
        for episode_step in range(self.args.episode_length):
            ...
            V_R, h_in_cri_R = self.agent_Reconn.get_value(state, h_in_cri_R) # [N_Reconn]
            V_C, h_in_cri_C = self.agent_Cannon.get_value(state, h_in_cri_C)
        ...
        if not evaluate:
            state = np.array(obs_n).flatten()
            V_R, _ = self.agent_Reconn.get_value(state, h_in_cri_R)
            V_C, _ = self.agent_Cannon.get_value(state, h_in_cri_C)
...
if __name__ == '__main__':
    ...
    parset.add_argument('--number', type=int, default=1000)
